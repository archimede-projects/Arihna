...
#### STEP 7 physical FAIL remediation — APPROVED 2026-09-05
...
5. **STEP 5 — functional alarm UI + notification/sound profiles: CLOSED, but physical delivery defect found later.** Exact runtime `310413e886e9d7d54ea6f86c3fc7849360069c49`; definitive gate `33952510472` / `101269810089`; API28 62/62, 0 failed, 0 skipped.
...
### 2026-09-06 — Galaxy S25 Home navigation, daily inspiration and Islamic visual identity correction APPROVED

Physical validation of prerelease `ui-s25-wow-fused-f61133c0-20260906` on the primary Samsung Galaxy S25 found a top-level navigation defect and two Home product-quality gaps. The validated runtime baseline is commit `f61133c0dcab8b1f1e7255967b81c280f064e3f9`; this specification update must precede all corresponding implementation changes.

**Top-level navigation correction.** The six bottom navigation destinations are true top-level destinations. From any screen reached either from the bottom bar or from Home quick actions, tapping **Home** must return to the existing Home destination immediately without requiring the Android system Back button. The same top-level navigation contract applies when switching among Orari, Qibla, Corano, Sveglie and Impostazioni: one tap selects the requested destination, duplicate copies are not accumulated, and state restoration must not make the selected tab unreachable. Dedicated regression coverage must exercise at minimum Home → Qibla → Home, Home → Sveglie → Home and Home → Posizione/Impostazioni → Home through visible UI controls.

**Daily inspiration interaction and religious-content discipline.** Replace the passive Home inspiration card with a tappable **Ispirazione del giorno** experience. The compact Home card shows a positive Quran verse or authentic hadith excerpt together with an explicit source/reference. Tapping the card opens a readable detail surface without navigating away from Home; the detail exposes a clear **Condividi** action using the Android system share sheet (`ACTION_SEND`, `text/plain`). Shared text must contain the displayed content and its source/reference. Daily content must come only from a small Arihna-owned curated set whose references are explicit and reviewable; Arihna must never fabricate a Quran attribution, hadith attribution or authenticity claim. Selection may rotate deterministically by local civil date and must remain stable for that date. Closing the detail returns to the unchanged Home state.

**Islamic visual identity correction.** The Home background must no longer read as a plain dark gradient. Preserve the approved premium dark-green/gold palette, hierarchy and legibility, but add a recognizably Islamic visual layer using subtle geometric/mashrabiya-inspired line work and/or architectural arch geometry rendered decoratively behind content. The ornament must remain low-contrast, non-textual and non-sacred: no Quranic text, divine names, generated Arabic calligraphy or decorative religious quotations in the background. It must not interfere with taps, accessibility semantics, prayer-time readability, scrolling or performance. The result should feel warmer, richer and more distinctly Islamic while remaining sober rather than busy or kitsch.

**Scope and gate.** Do not change prayer calculations, alarm scheduling, Fused location acquisition, GeoNames data, permission policy, package identity or frozen SDK values for this correction. Implementation must be one clean technical candidate whose direct parent is this spec commit. Before promotion, the exact candidate SHA must pass host unit regression, debug build, Android 9/API28 full instrumentation with zero failures/errors/skips, the modern location-permission matrix already required by the promoted baseline, frozen GeoNames/Adhan integrity checks, and the new navigation/inspiration UI regression. Only a green exact-SHA candidate may fast-forward `main`; the resulting APK must use the frozen persistent signer and be published as a GitHub prerelease for a new physical Galaxy S25 validation.
...
#### Galaxy S25 alarm heads-up + ringing controls correction — APPROVED 2026-09-05

Physical Galaxy S25 validation of runtime `a767eaa1f722a9c62ec3dbf3204791ee5b1f77a4` remains **FAIL** for the alarm presentation path. The alarm sound is delivered and the Arihna ringing activity is reachable, but on an unlocked device the first alarm surface appears as a small transient heads-up notification without visible `Interrompi` / `Rinvia` controls, and tapping the notification opens the Arihna full-screen ringing UI with only `Stop`. The user explicitly requires a substantially larger alarm popup with direct stop/snooze controls, plus faster diagnostic iteration.

Authorized corrective scope is strictly limited to:

- Keep the standards-based alarm notification path; do **not** request draw-over-other-apps / `SYSTEM_ALERT_WINDOW` or any unrelated permission.
- Always attach the existing alarm full-screen PendingIntent to the alarm notification via `setFullScreenIntent(...)`, including the Android fallback case where full-screen special access is unavailable. This is required so Android 13+ can present the alarm heads-up treatment associated with full-screen intents instead of Arihna downgrading itself to a normal transient notification.
- Preserve `CATEGORY_ALARM`, `IMPORTANCE_HIGH`, ongoing/non-auto-cancel behavior, foreground ringing service, exact-alarm policy, and the persistent Arihna debug signer.
- The heads-up / notification surface must expose both user actions: **Interrompi** and **Rinvia**. `Rinvia` remains the already-authorized 5-minute snooze behavior.
- The Arihna ringing activity must expose both **Interrompi** and **Rinvia 5 min**; no longer ship a ringing screen with only Stop.
- The Settings diagnostic action **Test sveglia** must schedule after **20 seconds** instead of 60 seconds. The same 20-second delay applies to Test Adhan so the paired hardware checks stay symmetric. No production alarm timing is changed.
- No changes to prayer-time mathematics, Qibla, location, prayer-reminder placement, personal-alarm editor semantics, selected ringtone behavior, recurrence, persistence, or any other screen are authorized in this round.
...
#### Galaxy S25 unlocked overlay + alarm-volume correction — APPROVED 2026-09-05

Physical Galaxy S25 validation of corrected runtime `925a3c45193d059122c2b65c09c33e9a22fb6b48` confirms that the ringing session is now stable: the alarm remains active, lock-screen/full-screen presentation works when Android permits it, and the standard expanded One UI alarm notification exposes `Interrompi` and `Rinvia`. The remaining user-visible limitation is that on an **unlocked** Galaxy S25 One UI initially renders the standard heads-up notification in its compact form, so the two actions are hidden until the user expands the notification. The user explicitly rejects that extra expansion gesture. The same physical session also established that the device alarm stream volume can be too low for the desired use.

This corrective round is authorized **only** for the two items below; all other Alarms, Prayer, Location, Qibla, persistence and scheduling behavior remains frozen.

**1. Unlocked alarm overlay — authorized special access**

- Arihna may declare `android.permission.SYSTEM_ALERT_WINDOW` solely to support a large alarm control surface while an already-validated alarm is actively ringing and the device is unlocked.
- The permission is special app access, never an ordinary runtime permission prompt. Arihna must check `Settings.canDrawOverlays(context)` and expose a user-triggered Settings action using `Settings.ACTION_MANAGE_OVERLAY_PERMISSION`. It must never claim overlay readiness while Android reports it unavailable.
- When overlay access is granted and the device is **unlocked**, the existing bounded ringing foreground service may attach one Arihna-owned `TYPE_APPLICATION_OVERLAY` window. The overlay is allowed only for the active ringing session, must be removed immediately on `Interrompi`, successful `Rinvia`, service destruction or terminal safety timeout, and must never remain as an always-on floating window.
- The overlay must be large enough to avoid the One UI compact-notification expansion gesture and must show the active alarm identity plus directly visible actions **Interrompi** and **Rinvia 5 min**. It should use the approved Arihna Green/Gold visual language and must not introduce unrelated content.
- When the device is locked, preserve the already-working lock-screen/full-screen alarm path; do not place the application overlay above the lock-screen ringing Activity. When overlay access is absent, preserve the stable OEM-safe standard `CATEGORY_ALARM` notification fallback rather than reintroducing the custom `RemoteViews` path that crashed on the physical S25.
- The standard ongoing notification remains part of the foreground-service contract and continues to contain `Interrompi` and `Rinvia` actions for the expanded shade/fallback path.

**2. Alarm-stream volume control in Settings**

- Add a `Volume sveglia` control under `Impostazioni → Sveglie e notifiche`. It must read the real Android `AudioManager.STREAM_ALARM` current/max volume and present it as a slider/percentage; it is not an Arihna-only software gain.
- User changes explicitly set the real alarm-stream volume through `AudioManager.setStreamVolume(AudioManager.STREAM_ALARM, ...)`, clamped to the platform minimum/maximum. The UI must state that this changes the phone's alarm volume globally.
- Do not request Notification Policy / Do-Not-Disturb access merely to move the normal alarm-volume slider. If the platform rejects a particular change, preserve the previous platform value and surface a controlled message instead of fabricating success.
- Do not add audio amplification beyond the device stream maximum, DSP boost, normalization, clipping, or a new audio dependency. Existing Adhan/system-ringtone `USAGE_ALARM` playback remains unchanged.
- Keep both real diagnostic buttons at **20 seconds**. They are the physical-device validation path for the new overlay and volume behavior.

**Required regression for this correction:** keep unfiltered JVM/build/API28 regression green with zero skipped tests; add focused Android coverage for manifest `SYSTEM_ALERT_WINDOW`, overlay access state/settings intent, overlay session lifecycle (created only when authorized+unlocked and removed on stop/snooze/destroy), absence of custom `RemoteViews`, and alarm-volume read/clamp/set behavior. The modern API36 gate must preserve denied/granted exact-alarm, notification and full-screen checks and add denied/granted overlay-access assertions without changing the exact-alarm semantics. The technical candidate must be a direct child of this specification commit, pass the exact-SHA gate, then be promoted by non-forced fast-forward and packaged with the persistent Arihna debug signer for another Galaxy S25 physical validation. No other screen redesign or feature is authorized by this correction.
...
#### Galaxy S25 personal-alarm compact UI redesign — APPROVED 2026-09-05

Physical Galaxy S25 validation of runtime `c79ec96e38c10eecce185d215e8abd7fd6ad3554` confirms that the unlocked ringing overlay correction is visually and functionally accepted. This new round is a separate **UI/UX-only correction for personal alarms** based on the user's direct comparison with Samsung Clock. The objective is to make personal alarms much more compact and to make editing feel like entering the alarm's own settings instead of managing actions from a large card.

Approved scope is limited to the personal-alarm list and the personal-alarm create/edit surface:

- Replace the current large personal-alarm cards with a compact, Samsung-inspired Arihna list treatment. This is interaction/layout inspiration only; do not copy Samsung assets or proprietary pixels.
- Each personal-alarm row must keep the alarm time visually dominant, retain the alarm label when present, show only concise recurrence/schedule information, and keep the enabled switch at the right.
- The complete alarm row is a direct tap target that opens that alarm's create/edit settings surface.
- Remove the visible `Modifica` and `Elimina` actions from every list row.
- Remove the ringtone/sound name from every list row. Sound remains editable inside the alarm settings surface.
- Keep an explicit `Nuova sveglia` entry point, but reduce the overall list chrome and vertical footprint: no oversized informational hero is required in this compact direction.
- For an existing alarm, destructive deletion is allowed only inside its edit/settings surface, not on the compact list row. A new alarm has no delete action.

The personal-alarm create/edit surface is reorganized for a cleaner premium Arihna presentation:

- Present it as a dedicated full-width/full-screen in-app settings surface rather than the current dense generic `AlertDialog` treatment, without introducing a navigation dependency.
- Keep the selected time as the main visual anchor and preserve the existing Android time-picker behavior.
- Keep weekday recurrence selection using the already-approved single-letter day controls.
- Keep the existing alarm-name value and ringtone picker/sound-enabled behavior; reorganize them into compact grouped rows with substantially less explanatory copy.
- Keep `Annulla` and `Salva` as immediately available actions. Existing alarms may expose `Elimina sveglia` inside this settings surface.
- Use the official Arihna Green/Gold palette with restrained derived neutral/emerald tones permitted by §4.2. Avoid the current heavy green-on-green presentation and unnecessary repeated labels; the result should have clearer hierarchy, stronger contrast and more visual polish while remaining legible on the Galaxy S25.
- Do not add new icons, image assets, dependencies, permissions or decorative content that is not required for this layout correction.

Frozen behavior for this round:

- Do not change custom-alarm recurrence semantics, one-shot auto-disable behavior, exact-alarm scheduling, reconciliation, snooze duration, selected ringtone persistence, sound-profile semantics, ringing service/audio, full-screen ringing, unlocked overlay, notification fallback, alarm-volume control, 20-second diagnostics, Prayer calculation, Prayer reminders, Location, Qibla, GeoNames, SDK levels, applicationId, dependencies or signing identity.
- Do not redesign the app-wide `Impostazioni` screen in this round. The user's Samsung screenshots in this approval refer to the per-alarm settings/editor experience.
- Existing non-personal Prayer-linked alarm UI placement remains unchanged.

Required validation:

- Add/update Compose instrumentation proving that a compact personal-alarm row opens editing when the row itself is tapped, the enabled switch remains independently operable, and list rows contain no `Modifica`, `Elimina` or ringtone/sound-name presentation.
- Verify the editor retains single-letter weekday controls, existing ringtone selection/switch behavior, save/cancel actions, and exposes deletion only for an existing alarm.
- Run unfiltered `testDebugUnitTest`, `assembleDebug`, Android 9/API28 connected regression with zero skipped tests, and the existing Android 16/API36 denied/granted exact-alarm, notification, full-screen and overlay matrix. Preserve frozen GeoNames SHA-256 and manifest/dependency policy.
- The technical candidate must be a **direct child of this specification commit**, pass the exact-SHA gate, then fast-forward non-forced to `main` and be packaged with the persistent Arihna debug signer as a new Galaxy S25 prerelease APK. Stop after publishing and verifying the APK for physical UI validation.

### General Settings premium visual simplification — APPROVED 2026-09-06

- Primary Galaxy S25 feedback: the general `Impostazioni` screen is visually too verbose, weakly grouped and heavy; the current dark green / grey card treatment and broad gold usage do not deliver the requested premium "wow" effect.
- Scope is strictly the general Settings presentation implemented by `LocationSettingsScreen`; do not alter Home, Prayer, Qibla, Quran, personal-alarm list/editor, navigation destinations, scheduling, location acquisition/persistence semantics, alarm ringing, overlay, lock-screen full-screen behavior, notifications, snooze, Adhan playback or alarm-volume behavior.
- Preserve every existing capability and action: Device location selection/rationale, manual offline GeoNames city search, controlled Location error states and remediation actions, notification/exact-alarm/full-screen/overlay management, real `STREAM_ALARM` volume control, the two 20-second diagnostic tests and cancellation.
- Reorganize the screen into three clear compact groups: `Posizione`, `Sveglie e notifiche`, and `Test rapidi`, beneath a single clean `Impostazioni` header. Remove temporary/internal copy such as `Pannello funzionale STEP 6 — la Home definitiva verrà costruita più avanti.` and remove redundant explanatory paragraphs from normal ready states.
- The Location ready presentation must become a compact summary: location name is primary, source/status/freshness are compact secondary indicators, timezone is secondary. Error and permission/service states must remain explicit and understandable, including their existing recovery actions; do not hide a real failure behind a decorative status.
- Device-location and manual-city controls remain available but visually compact. The rationale dialog still explains approximate location and local-only use before requesting permission.
- Alarm capabilities become compact rows with label, concise state and direct management action. `Popup sveglia` and all existing capability semantics remain unchanged.
- Alarm volume remains the real global Android alarm stream control. Keep an explicit concise warning that the slider changes the phone global alarm volume; no software amplification, DSP, extra audio dependency or DND special access.
- Diagnostic controls remain `Test sveglia (20 secondi)`, `Test Adhan (20 secondi)` and cancellation, but the long hardware-path explanation is removed from the normal card.
- Visual direction: use a screen-owned premium derived palette rather than the current heavy green/grey treatment. Keep the official Arihna identity as the source palette, with restrained gold accent instead of large gold text areas; use deep neutral/forest-charcoal surfaces, warm off-white primary text, muted sage secondary text, subtle outlines and accessible success/error states. Do not change the global app theme or the official palette constants for other screens.
...
### 2026-09-07 — Prayer Times / “Orari” fixed-screen audio selection APPROVED

Scope is intentionally narrow. The Home screen remains frozen in its current approved “Alba dorata” state. The Alarms/Sveglie screen and its editor remain completely out of scope for this iteration. The only product surface authorized by this decision is **Orari**, the second primary screen after Home.

**Fixed, non-scrollable Orari screen.** The Orari screen must be redesigned so its primary portrait layout on the Galaxy S25 fits within one fixed viewport and does not require vertical user scrolling. Do not use a vertically scrollable timeline/list as the normal interaction model for this screen. Prayer rows, day/date context, audio controls and bottom navigation must be compact enough to remain visible together without sacrificing legibility.

**Visual continuity with Home.** Orari must use the same approved “Alba dorata” visual language as Home: warm ivory/cream background, soft sage surfaces, restrained warm-gold highlights and dark forest/emerald typography/icons. Islamic geometric/architectural decoration may remain subtle and non-sacred. Orari must no longer read as a separate dark/heavy theme.

**Per-prayer audio state uses icons first.** Where Orari exposes an audio choice or state for a prayer, the compact surface should communicate the state primarily with recognizable icons rather than repeated labels. Approved examples: silent/muted = muted-speaker icon; Adhan = mosque icon; generic/system ringtone when available = bell/audio icon. Accessibility content descriptions remain required even when visible text is omitted.

**Multiple real Adhan choices.** Arihna must no longer present Adhan as a single undifferentiated sound. The user must be able to choose from a list containing multiple genuinely distinct Adhan audio choices. A visible list item must map to a real, distinct playable source; do not create fake variants that all resolve to the same bundled audio. Any new bundled audio must have explicit provenance/licensing suitable for free redistribution and must be recorded/verified in the repository before release.

**Persistence and playback.** The selected Adhan variant must be persisted as part of the prayer audio preference and the actual notification/alarm playback path must resolve and play that exact variant. Existing silent behavior must remain genuinely silent. Existing alarm-editor behavior is not part of this change and must not be redesigned as collateral work.
...
### 2026-09-07 — Global visual harmony, expanded Adhan catalogue and alarm-volume control APPROVED

Physical Galaxy S25 review of the promoted `882e434ea036ef9033608131b428e46b16b64d1e` build identified three product-level corrections that must be delivered together as one coherent system revision: top-level screens currently do not share one visual identity, the bundled Adhan catalogue is too small, and the alarm-volume slider in Settings is not the desired interaction.

**One Arihna visual language across the app.** The approved Home “Alba dorata” direction becomes the shared visual language for all six top-level destinations: Home, Orari, Qibla, Corano, Sveglie and Impostazioni. Use one shared set of Arihna-owned visual tokens based on warm ivory/cream, soft sage, restrained warm gold, and dark forest/emerald for legible text, icons and selected premium surfaces. Screen-specific functional emphasis is allowed, but independent incompatible palettes, near-black Settings-only backgrounds, system-theme-driven visual identity changes, and unrelated full-screen gradients are not. Backgrounds, cards, section labels, outlines, selected controls and bottom-navigation surroundings must read as parts of the same product family. Subtle low-contrast Islamic arch/geometric ornament may be shared across screens, but it must remain non-sacred, non-textual and never reduce readability, touch clarity or performance. No Quranic text, divine names, generated Arabic calligraphy or decorative religious quotations may be introduced as visual ornament. The already-approved Home hierarchy/content, Orari fixed-viewport behavior, Qibla compass behavior, Alarm behavior, Location behavior and top-level navigation semantics remain unchanged unless a small presentation-only adjustment is required for visual consistency.

**Expanded bundled Adhan catalogue.** Preserve the existing `Adhan classico`, `Adhan armonioso` and user-approved `Adhan breve` choices and expand the bundled catalogue to at least six selectable Adhan variants. Every new recording must come from a verifiable public-domain or open-license source suitable for redistribution, with source page, author/uploader where available, license, original filename, bundled filename and exact bundled SHA-256 documented under `docs/audio/`. Do not fabricate geographic, reciter, mosque or stylistic attribution from an ambiguous source. Existing stored variant ids must remain migration-safe. The same Arihna-owned catalogue must drive Orari selection and actual bundled Adhan playback; no choice may be UI-only or silently fall back to a different recording. APK-size impact must be measured and reported at the gate.

**Alarm volume interaction.** Remove the alarm-volume `Slider` from Impostazioni. Replace it with a compact stepper-style control containing an explicit decrease action, the current percentage/value as the central readout, and an explicit increase action. Each tap changes exactly one valid Android alarm-stream volume step through the existing `AlarmVolumeController`; decrease/increase actions are disabled at the platform minimum/maximum respectively. Preserve the honest explanation that this controls the phone’s global alarm volume. The UI must not pretend that this is an Arihna-only per-rule volume, and it must continue surfacing controlled platform failures.

**Architecture and scope guard.** Prefer shared visual tokens/components under `core/ui` rather than duplicating color constants in every feature. Do not change prayer-time calculation, prayer offsets/methods, explicit-current-location semantics, Fused location behavior, GeoNames data, Qibla mathematics/sensors, alarm scheduling/reconciliation, exact-alarm policy, notification policy, package id or SDK levels. Do not add a paid/network runtime dependency.

**Acceptance and SPEC-FIRST lineage.** This `PROJECT_SPEC.md` commit is the sole specification parent for the implementation candidate. The technical candidate must be a direct child of this exact spec commit. If any candidate fails its exact-SHA gate, every replacement must again be a direct child of this same spec commit, never a child of a failed candidate. The definitive exact-SHA gate must preserve the full host/unit/debug build regression, Android 9/API28 full instrumentation with zero failures/errors/skips, the modern API36 permission/capability matrix, frozen GeoNames and established Adhan integrity checks, package/SDK invariants and persistent-signing policy. It must additionally verify the expanded Adhan catalogue/provenance and bundled hashes, migration-safe existing ids, shared visual-system adoption across all six top-level destinations, absence of the Settings alarm-volume slider, and functional decrease/readout/increase volume controls. Only after the exact candidate is fully green may `main` advance by non-forced fast-forward and a persistent-signer Galaxy S25 prerelease APK be published. Physical Galaxy S25 validation remains the final checkpoint.
...
### 2026-09-10 — Corano: Muṣḥaf Tajwid page-first UX remediation — APPROVED

Real-device feedback from the verified Tajwid Beta APK showed that the first runtime is functionally correct but visually does not yet satisfy the requested Muṣḥaf reading experience. The observed defects are product defects, not merely cosmetic polish: one card per āyah destroys page continuity; the āyah marker is rendered in a separate `Text` and can fall onto an isolated line; the Tajwid mode header can show a misleading Hafs page context; the horizontal mode chips are clipped on the primary phone width; the always-visible six-rule legend and large disclaimer consume too much reading space.

Approved remediation:

- Keep the existing locally generated **`Colorazione tajwid · Beta / regole principali`** engine and its legal/provenance constraints unchanged. QCF V4 remains separately **BLOCKED** and must not be bundled.
- Change `Ḥafṣ Tajwid · Beta` from a sura-card text reader to a **page-first Muṣḥaf-style reader** driven by the already pinned authoritative Hafs `QuranData.Page` starts from `quran/quran-data.js`. There are exactly 604 pages; do not invent page boundaries.
- The visible current page in Tajwid must therefore be a real Hafs page number. Selecting a sura, Juz or Hizb opens the authoritative page containing that boundary. Swiping pages is RTL/right-to-left, matching the existing Hafs Muṣḥaf reader.
- Within a Tajwid page, Arabic text is visually continuous. Do **not** render one Material card per āyah. Use one paper-like page surface and continuous Arabic text blocks, splitting only where a genuine sura boundary requires a sura heading/Basmala.
- The āyah number/ornament must be appended **inline inside the same annotated Arabic text flow** as its āyah, never as a separate trailing `Text`. It must not become an orphan row below the verse. A Unicode Arabic end-of-āyah marker plus Arabic-Indic digits is acceptable when no separately redistributable traditional glyph asset is available.
- Apply Tajwid color spans only to the original pinned Uthmani text ranges. The displayed text content remains unchanged apart from UI-only separators/āyah markers and sura-heading/Basmala presentation; the engine must not mutate Quran source text.
- Preserve right-to-left shaping, diacritics and combining marks. Use restrained Quran-reading typography and page-like line spacing; do not claim that the system font is a QCF/traditional printed Muṣḥaf font.
- When a page begins a new sura, show a centered Arabic sura heading. Show the Basmala before the sura body where traditionally applicable, without duplicating Quran 1:1 and without adding it before Sura 9.
- Replace the permanently scrolling/clipped row of four reading-mode chips with a compact mode selector that is fully usable at Galaxy S25 width and exposes all four modes without partial clipping.
- In Tajwid mode, keep the Beta status clearly visible but compact. The explanatory disclaimer must no longer dominate the reading viewport.
- Make the six-rule Tajwid legend compact/collapsible instead of permanently consuming a full horizontal strip. All six labels/colors remain available to the user.
- Keep A−/A+, immersive `Lettura`, pinch zoom in fullscreen, S25 cutout/status-bar safety, sura/Juz/Hizb index, and bookmarks/recent state.
- Tajwid navigation/bookmark state becomes **page-based and separately namespaced** from ordinary Hafs/Warsh state. Existing Tajwid sura bookmarks/last/recent from the first Beta may be migrated to the corresponding authoritative sura-start pages; they must never be silently reinterpreted as ordinary Hafs bookmarks.
- Do not change Prayer, Location, Qibla, alarms, Home or unrelated visual behavior in this remediation.

Acceptance/gate additions:

- Unit or instrumentation coverage proves the 604 authoritative Hafs page starts are exposed without fallback/invented boundaries and map known boundaries correctly.
- Instrumentation on Tajwid proves an RTL page pager exists, a real page context is shown, mode selection is not a clipped horizontal strip, and the body uses a continuous page surface rather than one card per āyah.
