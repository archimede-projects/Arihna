...
CLASS=com.archimedeprojects.arihna.feature.alarms.platform.PopupModernAndroidTest
TEST_SRC=app/src/androidTest/java/com/archimedeprojects/arihna/feature/alarms/platform/PopupModernAndroidTest.kt

cp /tmp/PopupModernAndroidTest.kt "$TEST_SRC"
test -f "$TEST_SRC"
...
run_matrix() {
  ...
  local label="$3"
  local log="/tmp/popup-modern-${label}.txt"
...
rm -f "$TEST_SRC"
echo 'PERSISTENT_POPUP_MODERN_MATRIX PASS api=36 exact=denied+granted notifications=denied+granted fullscreen=always-attached headsup=custom actions=interrompi+rinvia'
