...
CLASS=com.archimedeprojects.arihna.feature.alarms.platform.CrashModernAndroidTest
TEST_SRC=app/src/androidTest/java/com/archimedeprojects/arihna/feature/alarms/platform/CrashModernAndroidTest.kt

cp /tmp/CrashModernAndroidTest.kt "$TEST_SRC"
test -f "$TEST_SRC"
...
run_matrix() {
  ...
  local label="$4"
  local log="/tmp/crash-modern-${label}.txt"
...
run_matrix false false false denied

adb shell appops set "$PKG" SCHEDULE_EXACT_ALARM allow
adb shell pm grant "$PKG" android.permission.POST_NOTIFICATIONS
adb shell appops set "$PKG" USE_FULL_SCREEN_INTENT allow
run_matrix true true true granted

rm -f "$TEST_SRC"
echo 'CRASH_REMEDIATION_MODERN_MATRIX PASS api=36 exact=denied+granted notifications=denied+granted fullscreen=denied+granted platform_notification=true'
