...
class SQLiteCityRepositoryAndroidTest {
    ...
    @Test
    fun everyBundledTimezoneIsSupportedByZoneIdOnApi28() {
        val database = databaseProvider.openReadOnly()
        val unsupported = mutableListOf<String>()
        var count = 0
        database.rawQuery("SELECT name FROM timezone ORDER BY id", null).use { cursor ->
            while (cursor.moveToNext()) {
        ...
        assertTrue("Unsupported API28 timezone ids: $unsupported", unsupported.isEmpty())
    }

    @Test
    fun finalAssetHasValidRowsAndFunctionalFtsGoldenMatches() {
        val database = databaseProvider.openReadOnly()
