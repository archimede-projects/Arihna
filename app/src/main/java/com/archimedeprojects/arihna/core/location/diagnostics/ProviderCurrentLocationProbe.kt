...
class ProviderCurrentLocationProbe(context: Context) {
    ...
    suspend fun runParallel() = coroutineScope {
        ...
        logProviderState(LocationManager.NETWORK_PROVIDER)
        logProviderState(FRAMEWORK_FUSED_PROVIDER)
    ...
    private fun logProviderState(provider: String) {
        val enabled = runCatching { locationManager.isProviderEnabled(provider) }.getOrDefault(false)
        val properties = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            runCatching { locationManager.getProviderProperties(provider) }
                .getOrNull()
                ?.let { props ->
                    " networkRequired=${props.hasNetworkRequirement()} cellRequired=${props.hasCellRequirement()} satelliteRequired=${props.hasSatelliteRequirement()} power=${props.powerUsage} accuracy=${props.accuracy}"
                }
                ?: " properties=<null>"
        } else {
            " properties=<api${Build.VERSION.SDK_INT}>"
        }
        LocationDiagnosticTrace.record(
            "AB_PROVIDER_STATE",
            "provider=$provider enabled=$enabled$properties enabledProviders=${enabledProvidersText()}",
        )
    }
    ...
    private suspend fun probeCurrent(provider: String) {
        ...
        LocationDiagnosticTrace.record(
            "AB_CURRENT_REQUEST",
            "provider=$provider elapsedMs=$requestElapsed wallMs=$requestWall",
        )
